...
try {
  User = (await import(path.join(__dirname, '../dist/models/User.js'))).default;
  Job = (await import(path.join(__dirname, '../dist/models/Job.js'))).default;
  Application = (await import(path.join(__dirname, '../dist/models/Application.js'))).default;
} catch (error) {
  console.error('❌ Error importing models:', error.message);
  console.log('💡 Make sure to compile TypeScript files first: npx tsc --project tsconfig.scripts.json');
  process.exit(1);
}
